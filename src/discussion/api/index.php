...
function updateTopic(PDO $db, array $data): void
{
    ...
    if (empty($setClauses)) {
        sendResponse(['success' => false, 'message' => 'At least one of subject or message is required.'], 400);
    }
...
function createReply(PDO $db, array $data): void
{
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $newReply = $db->prepare('SELECT id, topic_id, text, author, created_at FROM replies WHERE id = ?');
        $newReply->execute([$newId]);
        $reply = $newReply->fetch(PDO::FETCH_ASSOC);
